 ...
 * @since  1.6
 */
class MymercatosModel extends ListModel
{
	/**
	 * Constructor.
	 *
	...
	/**
	 * Get the master query for retrieving a list of mercatos subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->select(
			$this->getState(
				'list.select',
				'a.*,
				(SELECT MAX(`date`) from #__mymercato_dates WHERE mercato_id = a.id) AS last_visit,
				(SELECT count(`date`) from #__mymercato_dates WHERE mercato_id = a.id) AS nvisits
				')
		);
